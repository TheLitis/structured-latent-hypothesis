...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a direct separable-latent benchmark.")
    parser.add_argument("--output-dir", default="results/direct_separable_probe_v1")
...
def plot_direct_recon(results: dict, output_path: str | Path) -> None:
    worlds = results["worlds"]
    x_positions = range(len(worlds))
    figure, axis = plt.subplots(1, 1, figsize=(10, 4.8))
    for variant in results["variants"]:
        values = [results["summary"][world][variant]["test_recon_mse"]["mean"] for world in worlds]
    ...
    axis.set_ylabel("test recon mse")
    axis.set_title("Direct separable models across matched worlds")
...
def plot_residual_norm(results: dict, output_path: str | Path) -> None:
    worlds = results["worlds"]
    figure, axis = plt.subplots(1, 1, figsize=(10, 4.8))
    variants = [variant for variant in results["variants"] if "resid" in variant]
    for variant in variants:
        values = [results["summary"][world][variant]["residual_norm_train"]["mean"] for world in worlds]
        axis.plot(range(len(worlds)), values, marker="o", linewidth=2.0, label=variant)
    axis.set_xticks(list(range(len(worlds))))
    axis.set_xticklabels(worlds, rotation=30, ha="right")
    axis.set_ylabel("train residual norm")
    axis.set_title("Residual interaction norm across worlds")
    axis.grid(alpha=0.25)
    axis.legend(fontsize=8)
    figure.tight_layout()
    output_path = Path(output_path)
...
def main() -> None:
    ...
    plot_direct_recon(results, output_dir / "test_recon_by_world.png")
    plot_residual_norm(results, output_dir / "residual_norm_by_world.png")
    ...
    for world in results["worlds"]:
        ...
        if "additive_resid_selected" in results["summary"][world]:
            ...
            parts.append(f"additive_resid_selected `{value:.6f}` ({selection_summary(results, world, 'additive_resid_selected')})")
