...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run curvature-field and diagonal-defect candidates on the interaction ladder.")
    parser.add_argument("--output-dir", default="results/curvature_defect_probe_v1")
    ...
    parser.add_argument("--alphas", type=float, nargs="+", default=[0.00, 0.20, 0.35, 0.50, 0.75, 1.00])
...
def build_variant_recipes(args: argparse.Namespace) -> dict[str, dict]:
    return {
        ...
        "hankel_r4_selected": selected_recipe("additive_hankel_defect", 4, args.inner_train_fraction),
        "interaction_mlp_r4_selected": selected_recipe("additive_interaction_mlp", 4, args.inner_train_fraction),
...
def main() -> None:
    ...
    plot_metric(results, output_dir / "test_recon_vs_coupling.png", "test_recon_mse", "Curvature and diagonal defect candidates", "test recon mse")
    plot_metric(results, output_dir / "generalization_gap_vs_coupling.png", "generalization_gap", "Generalization gap across coupling", "test-train gap")
    plot_metric(results, output_dir / "residual_norm_vs_coupling.png", "residual_norm_train", "Train interaction norm", "train interaction norm")
