...
class DirectSeparableTest(unittest.TestCase):
    ...
    def test_curvature_hankel_hybrid_zero_components_gives_zero_interaction(self) -> None:
        model = AdditiveCurvatureHankelDecoder(grid_size=6, latent_dim=4, hidden_dim=24, output_dim=16, interaction_rank=2)
        with torch.no_grad():
            model.curv_basis.zero_()
            model.diag_second_diff.zero_()
        interaction = model.residual_grid()
    ...
    def test_curvature_hankel_hybrid_regularizer_has_expected_shape(self) -> None:
        model = AdditiveCurvatureHankelDecoder(grid_size=6, latent_dim=4, hidden_dim=24, output_dim=16, interaction_rank=2)
    ...
    def test_curvature_hankel_train_one_direct_smoke(self) -> None:
        config = DirectBenchmarkConfig(
            world="stepcurve_coupled_4.00_0.50",
            variant="curv_hankel",
            seed=3,
